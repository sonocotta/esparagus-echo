...
Application::Application()
{
  ...
  m_indicator_led = new NeopixelIndicatorLed();
#elif defined(LED_TYPE_GENERIC_ESP32)
  m_indicator_led = new GenericDevBoardIndicatorLed();
#else
  #error "No indicator LED type defined"
#endif
...
void Application::loop()
{
  ...
  while (true)
  {
    ...
    if (digitalRead(GPIO_TRANSMIT_BUTTON) == LOW)
    {
      ...
      while (millis() - start_time < 1000 || digitalRead(GPIO_TRANSMIT_BUTTON) == LOW)
      {
    ...
    unsigned long start_time = millis();
    while (millis() - start_time < 1000 || digitalRead(GPIO_TRANSMIT_BUTTON) == HIGH)
    {
